...
def all_accounts(request):
    ...
    return render(request, 'robo_app/all_accounts.html', context)
...
def all_lines(request):
    ...
    return render(request, 'robo_app/all_lines.html', context)
...
def account_details(request, account_id):
    ...
    return render(request, 'robo_app/account_details.html', context)
...
def line_details(request, account_id, line_id):
    ...
    return render(request, 'robo_app/line_details.html', context)
...
def signup_view(request):
    if request.method == 'POST':
        ...
        if password1 != password2:
            return render(request, 'robo_app/signup.html', {'error': 'Passwords do not match'})
        if User.objects.filter(username=username).exists():
            return render(request, 'robo_app/signup.html', {'error': 'Username already exists'})
        user = User.objects.create_user(username=username, password=password1)
        login(request, user)
        return redirect('dashboard')
    return render(request, 'robo_app/signup.html')
    ...
    try:
        ...
        for line in lines:
            ...
            line_service = LineService.objects.create(
                line=line,
                service=service,
                status='ACTIVE',  # Immediately activate for robo
...
def add_line_account_selection(request):
    ...
    return render(request, 'robo_app/add_line_account_selection.html', context)
